...
const Auth = () => {
  ...
  const onSubmit = async (data: AuthFormData) => {
    ...
    try {
      if (isSignUp) {
        const verificationResponse = await fetch('https://5000-firebase-kluster-backend-1758275720695.cluster-bg6uurscprhn6qxr6xwtrhvkf6.cloudworkstations.dev/api/verify-email', {
          method: 'POST',
          headers: {
        ...
        <Card className="glow-border bg-cyber-card/50 backdrop-blur-xl">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              <Button
                variant="outline"
                className="w-full border-cyber-border hover:bg-muted/50"
